...
export async function uploadDocumentForDataMgmtAction(
  ...
  try {
    ...
    if (!tenantId || !file || !docType || !code || !description || !category || !docCategory) {
      return {
    ...
    const getFileCategory = (ext: string) => {
      ...
      // Code
      if (['js', 'ts', 'jsx', 'tsx', 'html', 'css', 'json', 'xml', 'py', 'java', 'cpp', 'c', 'h'].includes(ext)) return 'code';
      // Default
      return 'other';
    };
    ...
    const path = `/tenant/${tenant.slug}/files/documents/${fileCategory}/${timestamp}-${safeName}` as `/${string}`;
    
    // Upload file to storage
    const uploadedPath = await uploadFile(path, file);
    ...
    const documentData = {
      id: documentId,
      name: file.name,
      type: file.type || `application/${fileExtension}` || "application/octet-stream",
      path: uploadedPath,
      size: file.size,
